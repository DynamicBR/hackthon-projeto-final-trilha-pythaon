...
@loja_route.route('/', methods=["GET", "POST"])
def exibir_loja():
    if 'usuario' not in session:
        return render_template(url_for('login.acessar'))
    ...
    produtos_loja = Produto.query.limit(5).all()
    total = session.get('carrinho', 0.0)

    return render_template('loja.html', produtos=produtos_loja, total=total)
